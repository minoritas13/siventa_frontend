...
const Home = () => {
  ...
  const [stats, setStats] = useState([
    { label: 'Total Aset', value: '0', icon: <Box size={20} />, bg: 'bg-[#991B1F]' },
    { label: 'Aset Aktif', value: '0', icon: <CheckCircle size={20} />, bg: 'bg-[#991B1F]' },
    { label: 'Aset Rusak', value: '0', icon: <Wrench size={20} />, bg: 'bg-[#991B1F]' },
    { label: 'Dipinjam', value: '0', icon: <Package size={20} />, bg: 'bg-[#991B1F]' },
  ]);
  ...
  useEffect(() => {
    const fetchAdminDashboard = async () => {
      try {
        ...
        const asetAktif = allItems.filter(item => item.kondisi === 'baik').length;
        const asetRusak = allItems.filter(item => item.kondisi === 'rusak').length;
        const totalDipinjam = allLoans.filter(loan => loan.status === 'dipinjam').length;

        setStats([
          { label: 'Total Aset', value: totalAset.toString(), icon: <Box size={20} />, bg: 'bg-[#991B1F]' },
          { label: 'Aset Aktif', value: asetAktif.toString(), icon: <CheckCircle size={20} />, bg: 'bg-[#991B1F]' },
          { label: 'Aset Rusak', value: asetRusak.toString(), icon: <Wrench size={20} />, bg: 'bg-[#991B1F]' },
          { label: 'Dipinjam', value: totalDipinjam.toString(), icon: <Package size={20} />, bg: 'bg-[#991B1F]' },
        ]);
        ...
        const mappedPinjaman = allLoans.map((loan) => {
          ...
          if (loan.status === 'selesai') statusColor = 'bg-[#53EC53]'; // Hijau
          if (loan.status === 'dipinjam') statusColor = 'bg-[#FBBF24]'; // Kuning
          ...
          return {
            ...
            kode: loan.item?.kode || "-",
            barang: loan.item?.name || "Aset Terhapus",
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50 font-sans">
      ...
      <main className="flex-1 p-4 md:p-10 pt-20 md:pt-10 overflow-x-hidden">
        ...
        <header className="mb-8">
          ...
          <p className="text-gray-500 text-xs md:text-sm">Pantau dan kelola seluruh inventaris SIVENTA.</p>
        </header>

        {/* Section Ringkasan Aset */}
        <section className="mb-10">
          <h2 className="text-lg md:text-xl font-bold text-black mb-6">Ringkasan Global</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((stat, index) => (
              <div key={index} className={`${stat.bg} text-white rounded-xl p-5 flex items-center gap-4 shadow-sm transition-transform hover:scale-105`}>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="p-5 border-b border-gray-100 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-gray-800">Log Aktivitas Pinjaman</h3>
            </div>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[900px]">
              ...
              <tbody className="divide-y divide-gray-100">
                {loading ? (
                  <tr>
                    <td colSpan="6" className="text-center py-10 text-gray-400">Memuat data aktivitas...</td>
                  </tr>
                ) : pinjaman.length > 0 ? (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-gray-200 overflow-hidden shrink-0 border border-gray-100">
                            <img 
                              src={item.foto || `https://ui-avatars.com/api/?name=${item.staff}&background=random`} 
                              alt="avatar" 
                              className="w-full h-full object-cover"
                            />
                          </div>
                          <div>
                            <p className="text-sm font-bold text-gray-800">{item.staff}</p>
                            <p className="text-[11px] text-gray-400 font-medium">{item.divisi}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-[11px] font-medium text-gray-500 uppercase">{item.kode}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-700">{item.barang}</td>
                      <td className="px-6 py-4 text-[12px] text-gray-600">{item.pinjam}</td>
                      <td className="px-6 py-4 text-[12px] text-gray-600">{item.kembali}</td>
                      <td className="px-6 py-4 text-center">
                        <span className={`${item.color} ${item.text} text-[10px] px-4 py-1.5 rounded-full shadow-sm`}>
                          {item.status}
                        </span>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-10 text-gray-400">Belum ada aktivitas pinjaman.</td>
                  </tr>
                )}
              </tbody>
            </table>
